...
void CostmapLayer::updateWithMax(costmap_2d::Costmap2D& master_grid, int min_i, int min_j, int max_i, int max_j)
{
  if (!enabled_)
    return;
  // master_array为旧master，临时指针与全局指针指向同一块内存：costmap_
  ...
  for (int j = min_j; j < max_j; j++)
  {
    ...
    for (int i = min_i; i < max_i; i++)
    {
      ...
      // 如果旧mast地图中的当前栅格的代价值为未知，或者当前代价值小于新的值
      if (old_cost == NO_INFORMATION || old_cost < costmap_[it])
        // costmap_不断接收订阅slam发布的静态地图数据，将静态层的地图数据复制给Master Map
                                           ...
                                           int max_i, int max_j)
{
  if (!enabled_)
    return;
  // master为旧master，临时指针与全局指针指向同一块内存：costmap_
  ...
  for (int j = min_j; j < max_j; j++)
  {
    ...
    {
      // 新maste地图更新旧master地图
      // notice: costmap_不断接收订阅slam发布的静态地图数据，将静态层的地图数据复制给Master Map
      master[it] = costmap_[it];
      it++;
    }
  }
}// 销毁指针，但是指针指向的内存数据得到更新
